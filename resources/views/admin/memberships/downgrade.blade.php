                ...
                <div class="row form-group">
                        ...
                        <select onchange="getExpiry()"
                            ...
                            @foreach ($pricelists as $pricelist)
                                <option value="{{ $pricelist->id }}"
                                    {{ old('service_pricelist_id') == $pricelist->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-3">
                        ...
                        <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text"
                            name="end_date" id="end_date" value="{{ $membership->end_date }}" required
            ...
            @foreach ($accounts as $id => $name)
                ...
                $('#{{ $name }}').on('keyup', function() {
                    ...
                    if (parseFloat($('#received_amount').val()) >= parseFloat($("#net_amount").val())) {
                        var diff = parseFloat($('#net_amount').val()) - parseFloat($('#received_amount').val());
                        // parseFloat($('#received_amount').val("{{ $invoice->payments_sum_amount }}"));
                        $('#amount_pending').val(diff);
                    }
                })
            @endforeach
        }
