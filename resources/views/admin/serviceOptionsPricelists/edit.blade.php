...
<div class="card">
    <div class="card-header">
        <h5>{{ trans('global.edit') }} {{ trans('cruds.serviceOptionsPricelist.title_singular') }}</h5>
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.service-options-pricelists.update", [$serviceOptionsPricelist->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_option') ? 'is-invalid' : '' }}" name="service_option_id" id="service_option_id" required>
                    @foreach($service_options as $id => $entry)
                        <option value="{{ $id }}" {{ (old('service_option_id') ? old('service_option_id') : $serviceOptionsPricelist->service_option->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pricelist') ? 'is-invalid' : '' }}" name="pricelist_id" id="pricelist_id" required>
                    @foreach($pricelists as  $pricelist)
                        <option value="{{ $pricelist->id }}" {{ (old('pricelist_id') ? old('pricelist_id') : $serviceOptionsPricelist->pricelist->id ?? '') == $pricelist->id ? 'selected' : '' }}>{{ $pricelist->amount.'@'.$pricelist->service->name .' - '.$pricelist->service->service_type->name  ?? '-'}}</option>
                    @endforeach
                </select>
