...
<div class="card">
    <div class="card-header">
        <h5>{{ trans('global.edit') }} {{ trans('cruds.document.title_singular') }}</h5>
    </div>
        ...
        <form method="POST" action="{{ route("admin.documents.update", [$document->id]) }}" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                        @foreach($employees as $id => $entry)
                            <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $document->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $document->description) }}</textarea>
