            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h6 class="text-center"> {{ request('from') ?? date('Y-m-01') }} : {{ request('to') ?? date('Y-m-t') }}</h6>
                    </div>
                </div>
            </div>
        </div>
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($accounts as $acc)
                                                <tr>
                                                    ...
                                                    @foreach ($service_payments as $key => $payment)
                                                        <td>
                                                            {{ number_format($payment->where('account_id',$acc->id)->sum('amount')) }}</td>
                                                    @endforeach
                                                    <td>
        ...
        <div class="row">
            ...
            @foreach ($service_refunds as $key => $refund)
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <strong>{{ $key }} - {{ trans('global.outcome') }} : <span class="text-white">{{ number_format($refund->sum('amount')) }} EGP ({{ $refund->count().' Refund' }})</span> </strong>
                        </div>
                        <div class="card-body table-responsive">
                            <table class="table table-striped table-hover table-bordered zero-configuration">
                                <thead>
                                        ...
                                        <tr>
                                            <td>
                                                <a href="{{ route('admin.invoices.show', $ref->invoice_id) }}"
                                                    target="_blank">{{ $ref->invoice->membership->member->branch->invoice_prefix . ' ' . $ref->invoice_id }}
                                                </a>
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.members.show',$ref->invoice->membership->member_id) }}">
                                                    <b class="d-block">
                                                        {{ $ref->invoice->membership->member->branch->member_prefix . $ref->invoice->membership->member->member_code ?? '' }}
