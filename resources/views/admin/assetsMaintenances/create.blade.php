...
<div class="card">
    <div class="card-header">
        <h5>{{ trans('global.create') }} {{ trans('cruds.assetsMaintenance.title_singular') }}</h5>
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets-maintenances.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            @livewire('get-accounts-component')
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('maintence_vendor') ? 'is-invalid' : '' }}" name="maintence_vendor_id" id="maintence_vendor_id" required>
                    @foreach($maintence_vendors as $id => $entry)
                        <option value="{{ $id }}" {{ old('maintence_vendor_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
