...
@section('content')
    ...
    <div class="form-group row">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                    ...
                    @if (auth()->user()->roles[0]->title != 'Sales' || auth()->user()->roles[0]->title != 'Sales Manager')
                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal" data-target="#trainer_reminder">
        ...
        {{-- @foreach ($attendances as $att)
            @if ($att->created_at->format('Y-m-d') == date('Y-m-d') && $att->sign_out == null)
                <div class="col-md-3">
                    {!! Form::open(['method' => 'PUT', 'url' => route('admin.membership-attendances.update', $att->id)]) !!}
                    <input type="hidden" name="sign_out" value="{{ date('H:i:s') }}">
                    <button class="btn btn-danger btn-block font-weight-bold" type="submit">
                        <i class="fa fa-sign-out"></i> {{ trans('cruds.membershipAttendance.fields.sign_out') }}
                        ( {{ trans('global.time') }} : {{ date('g:i A', strtotime($att->sign_in)) }} )
                    </button>
                    {!! Form::close() !!}
                </div>
            @endif
            ...
            <div class="card">
                <div class="card-body">
                    <div class="nav nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        <a class="nav-link active" id="basic_data-tab" data-toggle="pill" href="#basic_data"
                            role="tab" aria-controls="basic_data" aria-selected="true">
    ...
    <div class="modal fade" id="trainer_reminder" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.take-trainer-reminder',[$member->id]) }}" method="post" class="trainer_reminder_form">
                    @csrf
                    <div class="modal-body">
                        <div class="alert alert-info font-weight-bold">
                            <i class="fa fa-exclamation-circle"></i> {{ trans('global.if_empty_due_date') }} .
