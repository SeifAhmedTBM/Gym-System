...
<div class="card">
    <div class="card-header">
        <h5>{{ trans('global.edit') }} {{ trans('cruds.news.title_singular') }}</h5>
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.news.update", [$news->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="cover">{{ trans('cruds.news.fields.cover') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.news.fields.cover_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="required" for="description">{{ trans('cruds.news.fields.description') }}</label>
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $news->description) }}</textarea>
            ...
            <div class="form-group">
                <label class="required" for="section_id">{{ trans('cruds.news.fields.section') }}</label>
                <select class="form-control select2 {{ $errors->has('section') ? 'is-invalid' : '' }}" name="section_id" id="section_id" required>
                    @foreach($sections as $id => $entry)
                        <option value="{{ $id }}" {{ (old('section_id') ? old('section_id') : $news->section->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
@if(isset($news) && $news->cover)
      var file = {!! json_encode($news->cover) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview)
      file.previewElement.classList.add('dz-complete')
